    ...
    if (status == 'not attempted') {
       setLessonStatus('incomplete');
        ...
        if (lastpage.length > 0 && lastpage != '' && lastpage != 'null' ) {
            if (lastpage != startingPage && shouldPrompt) { // don't prompt if that's where they're going, anyway
...
function setSaveData( data ) {
    if (STANDALONE) {
        ShellStorage.set("cmi.suspend_data", data );
    } else {
...
function getSaveData() {
	//console.log("StandaloneVA: ");
	//console.log(STANDALONE);
    if (STANDALONE) {
        return ShellStorage.get("cmi.suspend_data","null");
    } else {
...
function getLessonLocation() {
	if (!STANDALONE) {
		...
		return ShellStorage.get("cmi.location", null);
	}
}
...
function setLessonLocation(location) {
	if (!STANDALONE) {
        ...
        ShellStorage.set("cmi.location", location);
    }
